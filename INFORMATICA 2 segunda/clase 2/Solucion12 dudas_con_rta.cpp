...
int main(int argc, char *argv[]) {
	...
	do
	{
		scanf("%d", (ptr_v+*p_cant));              //duda profe : no entiendo lo que esta dentro del segundo parentesis 
		/*Rta. Es la posicion (muchas veces lo llamamos i) que se va incrementando 1 en 1 mas abajo. Esto es porque necesitamos 
		contar cuantos valores se ingresan  porque el fin de la carga es cero o 10 que es el maximo */
		if(*(ptr_v+*p_cant)!=0)
		{
			*p_cant+=1;                            //duda profe:esto no incrementaria la direccion de memoria ,no deberia tener parentesis para 
		}											//incrementar el valor de cantidad? Rta: incrementa el valor que esta en *p_cant
		else
		{
			bandera=false;
		}
